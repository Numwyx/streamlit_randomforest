...
setwd("E:\\咸鱼\\2024.4.13以后\\23.小庞统计机器学习分类模型2.0版本\\小庞统计分类机器学习模型2.0版本")
...
data$Result = factor(data$Result,levels = c(0,1),labels = c('No','Yes'))    #Result为分类结局
...
###################################训练集单因素分析##############################################################
tab3 = CreateTableOne(vars = myVars,strata = 'Result',#修改因变量名称，strata指定因变量，这里因变量为Result
...
##################################################################################
#举例NNET模型SHAP解释，若最优模型是其他模型，替换其中的模型即可
###################################################################################
...
sv_dependence(shap_value$Yes,v = "Age",#指定第一个变量自己需要绘制的变量
              color = "#3b528b",
